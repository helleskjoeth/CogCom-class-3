...
getwd()
sarah_list <- 
list.files(path = "C:/Users/helle/OneDrive/Documents/AU/1st semester/Cognition and Communication/Class/Class 3/data", full.names = T)
sarah_data
...
#```{R}
#sarah_data <- map_df(sarah_list,function(x) read.CLAN.file(x))
```
...
?subset()
?select
relevant_sarah_data <- select(sarah_data,"Speaker", "Gloss", "Date")
```
...
```{R}
?write.csv
write.csv(relevant_sarah_data, "sarah_data_clean.csv")
getwd()
```
...
sarah_data_1 <- read.csv("sarah_data_clean.csv", header=T)
```
...
str(sarah_data_1)
as.factor("Speaker")
sarah_data_1$Gloss <- as.character(sarah_data_1$Gloss)
sarah_data_1$Date <- anytime::anydate(sarah_data_1$Date)
str(sarah_data_1)
```
...
```{R}
sarah_data_1 = filter(sarah_data_1, Speaker == "MOT" | Speaker == "CHI")

write.csv(sarah_data_1, "sarah_data_2.csv")
```
HELP: https://r4ds.had.co.nz/transform.html#dplyr-basics

----Part 3: Cleaning strings
```{r}
sarah_data_2 <- read.csv("sarah_data_2.csv", header = T)
